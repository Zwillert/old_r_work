...
##### Process Data #####
  ...
  dooit = function(){
    ...
    output$plot0 <- renderPlotly({
      ...
      pl = ggplot() + geom_point(data = dtr, aes(Dim1, Dim2, col = party_unity, Percent_of_votes_in_line_with_party = Percent_of_votes_in_line_with_party, State = state_name, Name = person_name)) + scale_colour_gradient2(low = "blue", high = "red", guide = "colourbar")
